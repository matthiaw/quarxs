...
public class NodeTypeClassBuilder implements WikiComponentBuilder
{
    ...
    public List<DocumentReference> getDocumentReferences()
    {
        ...
        try {
            Query query =
                queryManager.createQuery("SELECT doc.space, doc.name FROM Document doc, doc.object(" + Constants.SPACE
                    + "." + NodeType.CLASS + "Class) AS obj", Query.XWQL);
            List<Object[]> results = query.execute();
            for (Object[] result : results) {
                references.add(new DocumentReference(getXWikiContext().getDatabase(), (String) result[0],
                    (String) result[1]));
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public List<WikiComponent> buildComponents(DocumentReference reference) throws WikiComponentException
    {
        ...
        DocumentReference classReference =
            new DocumentReference(getXWikiContext().getDatabase(), Constants.SPACE, NodeType.CLASS + "Class");
        ...
        try {
            ...
            if (!rightService.hasAccessLevel("admin", doc.getAuthor(), "XWiki.XWikiPreferences", getXWikiContext())) {
                throw new WikiComponentException(String.format("Failed to building " + NodeType.CLASS
                    + "Class components from document " + " [%s], author [%s] doesn't have admin rights in the wiki",
            ...
            for (final BaseObject obj : doc.getXObjects(classReference)) {
                ...
                n.setPrettyId(doc.getSpace() + "." + doc.getName() + "." + obj.getStringValue("name") + " ("
                    + obj.getGuid() + ")");
                if (obj.getStringValue("width") != null) {
                    if (!obj.getStringValue("width").equals("")) {
            ...
            throw new WikiComponentException(String.format("Failed to build " + NodeType.CLASS
                + "Class components from document [%s]", reference.toString()), e);
        }
