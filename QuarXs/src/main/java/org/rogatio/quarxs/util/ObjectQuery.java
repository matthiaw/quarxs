...
public class ObjectQuery {
	...
	private static String create(String className, String... values) {
		...
		return "select " + select + " from BaseObject as obj, " + from + " where obj.className='" + className + "' " + where;
	}

	public static Edge getEdge(String objectPrettyId, QueryManager queryManager, XWikiContext context) {
		// System.out.println(objectPrettyId);
		...
		// try {
		// Query query =
		// queryManager.createQuery(ObjectQuery.create(Constants.SPACE + "." +
		// Edge.CLASS + "Class", "prettyid", "edgetype", "nodesource",
		// "nodetarget"), Query.HQL);
		...
		try {
			...
			if (objectPrettyId.contains(". (")) {
				Query query = queryManager.createQuery(ObjectQuery.create(Constants.SPACE + "." + Edge.CLASS + "Class", "prettyid", "edgetype", "nodesource", "nodetarget"), Query.HQL);
				results = query.execute();
				WITHOUTLABEL = true;
			} else {
				Query query = queryManager.createQuery(ObjectQuery.create(Constants.SPACE + "." + Edge.CLASS + "Class", "prettyid", "label", "edgetype", "nodesource", "nodetarget"), Query.HQL);
				results = query.execute();
				WITHOUTLABEL = false;
			}
			// Query query =
			// queryManager.createQuery(ObjectQuery.create(Constants.SPACE + "."
			// + Edge.CLASS + "Class", "prettyid", "label", "edgetype",
			// "nodesource", "nodetarget"), Query.HQL);
			// List<Object[]> results = query.execute();
			// System.out.println(results.size());
			for (Object[] result : results) {
				if (result[0].toString().contains(objectGUID)) {
					...
					// source.setLabel(PrettyIdConverter.getNodeName(result[3].toString()));

					if (source!=null) {
					if (!source.getPrettyId().equals(sourcedesc)) {
						System.out.println("In Edge '" + e.getPrettyId() + "' is referenced Source-Node '" + source.getPrettyId() + "' with old '" + result[3].toString() + "'");
						// context.getWiki().saveDocument(doc, context);
						// source.setLabel(PrettyIdConverter.getNodeName(result[3].toString()));
					}//else {
//						System.out.println("In Edge '" + e.getPrettyId() + "' is referenced Source-Node NOT found. Should be '"+targetdesc+"'");						
//					}
					}
					...
					if (target != null) {
						if (!target.getPrettyId().equals(targetdesc)) {
							System.out.println("In Edge '" + e.getPrettyId() + "' is referenced Target-Node '" + target.getPrettyId() + "' with old '" + result[4].toString() + "'");
							// context.getWiki().saveDocument(doc, context);
							// target.setLabel(PrettyIdConverter.getNodeName(result[4].toString()));
						}
					} //else {
//						System.out.println("In Edge '" + e.getPrettyId() + "' is referenced Target-Node NOT found. Should be '"+targetdesc+"'");						
//					}
					// target.setLabel(PrettyIdConverter.getNodeName(result[4].toString()));
					e.setSource(source);
					e.setTarget(target);
	...
	public static List<Node> getNodes(QueryManager queryManager, XWikiContext context) {
		...
		try {
			Query query = queryManager.createQuery(ObjectQuery.create(Constants.SPACE + "." + Node.CLASS + "Class", "prettyid", "label", "nodetype"), Query.HQL);
			List<Object[]> results = query.execute();
			for (Object[] result : results) {
	...
	public static Node getNode(String objectPrettyId, QueryManager queryManager, XWikiContext context) {
		...
		try {
			...
			List<Object[]> results = query.execute();
			for (Object[] result : results) {
//				System.out.println(result[0].toString().contains(objectGUID));
				if (result[0].toString().contains(objectGUID)) {
//					System.out.println(objectGUID+"=!="+result[0]);
					DefaultNode n = new DefaultNode();
					n.setPrettyId(result[0].toString());
					n.setLabel(result[1].toString());
					if (!PrettyIdConverter.getNodeName(objectPrettyId).equals(result[1].toString())) {
...
//					System.out.println(objectGUID+"=?="+result[0]);
	...
	public static EdgeType getEdgeType(String objectPrettyId, QueryManager queryManager) {
		...
		try {
			Query query = queryManager.createQuery(create(Constants.SPACE + "." + EdgeType.CLASS + "Class", "prettyid", "name", "connection", "defaultlabel"), Query.HQL);
			List<Object[]> results = query.execute();
			for (Object[] result : results) {
				if (result[0].toString().contains(objectGUID)) {
	...
	public static NodeType getNodeType(String objectPrettyId, QueryManager queryManager) {
		...
		try {
			Query query = queryManager.createQuery(create(Constants.SPACE + "." + NodeType.CLASS + "Class", "prettyid", "name", "iconurl", "entity"), Query.HQL);
			List<Object[]> results = query.execute();
			for (Object[] result : results) {
				if (result[0].toString().contains(objectGUID)) {
	...
	public static NodeType getNodeTypeDefault(QueryManager queryManager) {
		...
		try {
			...
			List<Object[]> results = query.execute();
			for (Object[] result : results) {
				if (result[0].toString().contains(".Default ")) {
					DefaultNodeType nt = new DefaultNodeType();
					nt.setPrettyId(result[0].toString());
					nt.setName(result[1].toString());
					nt.setIconUrl(result[2].toString());
					nt.setEntity(result[3].toString());
					String objectGUID = nt.getPrettyId().substring(nt.getPrettyId().indexOf("(") + 1, nt.getPrettyId().length() - 1);
					nt.setGuid(objectGUID);
					return nt;
				}
			}
